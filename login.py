...
async def login_main(page: ft.Page):
        ...
        content=ft.Column(
            [
                ft.Row(
                    [
                        ft.Image(
                            src="/assets/QuasarBizz_ERP-amplio.png",
                            width=200,
                            height=80,
                        )
                    ],
    ...
    async def login_click(e):
        ...
        try:
            ...
            if isinstance(resultado, tuple) and resultado[1] == 1:
                # Login exitoso: limpiar página y cargar main_app
                page.clean()
    ...
    def on_disconnect(e):
        print("Cliente desconectado, cerrando Python...")
        os._exit(0)

    page.on_disconnect = on_disconnect
    # Lanzar animación de entrada correctamente
    page.run_task(animar_entrada)  # <--- pasar la función, no coroutine instanciada
